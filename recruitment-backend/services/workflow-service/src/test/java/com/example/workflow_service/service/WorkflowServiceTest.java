 ...
 * [N4] positionId null trong steps → bỏ qua trong danh sách, không gọi
 * userService
...
class WorkflowServiceTest {
        ...
        @MockitoBean
        private com.example.workflow_service.messaging.NotificationProducer notificationProducer;
        @MockitoBean
        private com.example.workflow_service.messaging.RecruitmentWorkflowProducer workflowProducer;
        @MockitoBean
         ...
         * Nhánh [N3]: Steps là danh sách rỗng → bỏ qua validation, tạo workflow không
         * có steps
         *
         ...
         * Nhánh [N6]: HierarchyOrder tăng dần (step sau có level thấp hơn step trước) →
         * CustomException
         *
         ...
         *
         * Bug bị bắt: Nếu điều kiện so sánh bị sai dấu (< thay vì >) → không phát hiện
         * sai thứ tự.
         */
        @Test
         ...
         * Nhánh [N6]: HierarchyOrder bằng nhau (step2 = step1) → PHẢI HỢP LỆ (chỉ
         * reject khi TĂNG)
         *
         ...
         * Nhánh [N7]: Steps hợp lệ (hierarchyOrder giảm dần) → workflow và steps được
         * lưu đúng
         *
         * Bug bị bắt:
         ...
         *
         * Bug bị bắt: Nếu code trả về null thay vì ném exception → null pointer ở
         * caller
         */
        @Test
         ...
         */
        @Test
        @SuppressWarnings("unchecked") // Java type erasure: PaginationDTO.getResult() trả về Object, cast an toàn vì
                                       // service luôn trả về List<WorkflowResponseDTO>
        ...
        /**
         * Test Case ID: WF-TC12A
         * Nhánh getAll() còn thiếu: workflow không có steps, workflow có steps, và step
         * có approverPositionId = null
         *
         * Mục tiêu: phủ các lambda stream trong getAll() mà các test DB hiện tại chưa
         * chạm đủ.
         */
        @Test
        @SuppressWarnings("unchecked")
        @DisplayName("[WF-TC12A] getAll() - phủ nhánh stream với workflow null steps và step null approverPositionId")
        void tc12a_getAll_coversStreamBranchesWithNullAndNonNullSteps() {
                WorkflowService targetService = AopTestUtils.getTargetObject(workflowService);
                WorkflowRepository originalRepository = (WorkflowRepository) ReflectionTestUtils.getField(targetService,
                                "workflowRepository");
                WorkflowRepository mockedRepository = Mockito.mock(WorkflowRepository.class);
                ReflectionTestUtils.setField(targetService, "workflowRepository", mockedRepository);

                try {
                        Workflow workflowWithoutSteps = new Workflow();
                        workflowWithoutSteps.setName("Workflow Không Có Steps");
                        workflowWithoutSteps.setType(WorkflowType.REQUEST);
                        workflowWithoutSteps.setIsActive(true);
                        workflowWithoutSteps.setSteps(null);

                        Workflow workflowWithNullStep = new Workflow();
                        workflowWithNullStep.setName("Workflow Có Step Null");
                        workflowWithNullStep.setType(WorkflowType.REQUEST);
                        workflowWithNullStep.setIsActive(true);
                        WorkflowStep nullStep = new WorkflowStep();
                        nullStep.setStepOrder(1);
                        nullStep.setApproverPositionId(null);
                        nullStep.setIsActive(true);
                        nullStep.setWorkflow(workflowWithNullStep);
                        workflowWithNullStep.setSteps(new java.util.LinkedHashSet<>(List.of(nullStep)));

                        Workflow workflowWithValidStep = new Workflow();
                        workflowWithValidStep.setName("Workflow Có Step Hợp Lệ");
                        workflowWithValidStep.setType(WorkflowType.OFFER);
                        workflowWithValidStep.setIsActive(true);
                        WorkflowStep validStep = new WorkflowStep();
                        validStep.setStepOrder(1);
                        validStep.setApproverPositionId(10L);
                        validStep.setIsActive(true);
                        validStep.setWorkflow(workflowWithValidStep);
                        workflowWithValidStep.setSteps(new java.util.LinkedHashSet<>(List.of(validStep)));

                        Pageable pageable = PageRequest.of(0, 10);
                        when(mockedRepository.findByFilters(any(), any(), any(), any(),
                                        any(Pageable.class)))
                                        .thenReturn(new PageImpl<>(
                                                        List.of(workflowWithoutSteps, workflowWithNullStep,
                                                                        workflowWithValidStep),
                                                        pageable, 3));
                        when(userService.getPositionNamesByIds(anyList(), anyString()))
                                        .thenReturn(Map.of(10L, "Manager"));

                        PaginationDTO result = targetService.getAll(null, null, null, null, pageable);
                        List<WorkflowResponseDTO> list = (List<WorkflowResponseDTO>) result.getResult();

                        assertThat(list).hasSize(3);
                        assertThat(list.stream().anyMatch(w -> w.getName().equals("Workflow Không Có Steps"))).isTrue();
                        assertThat(list.stream().anyMatch(w -> w.getName().equals("Workflow Có Step Null"))).isTrue();
                        assertThat(list.stream().anyMatch(w -> w.getName().equals("Workflow Có Step Hợp Lệ"))).isTrue();
                } finally {
                        ReflectionTestUtils.setField(targetService, "workflowRepository", originalRepository);
                }
        }
         ...
         * Nhánh [N14]: Tên mới = tên hiện tại → KHÔNG kiểm tra trùng, không ném
         * exception
         *
         * Bug bị bắt: Nếu code dùng `.equals()` sai chiều → ném exception sai khi đặt
         * cùng tên
         */
        @Test
         ...
         * Nhánh [N16]+[N17]+[N19]: Update hợp lệ — description mới, isActive=false,
         * updatedBy ghi đúng
         *
         * Bug bị bắt:
         ...
         * Nhánh [N21]: ID hợp lệ → Soft delete: isActive=false, record phải VẪN CÒN
         * trong DB
         *
         * Bug bị bắt:
        ...
        /**
         * Test Case ID: WF-TC19
         * Nhánh [N17]: dto.getSteps() không null nhưng rỗng → clear steps cũ, không
         * thêm step mới
         */
        @Test
        @DisplayName("[WF-TC19][N17] update() - steps rỗng → xóa steps cũ, không tạo step mới")
        void tc19_update_emptySteps_clearsExistingSteps() {
                CreateWorkflowDTO createDto = buildCreateDto("Workflow Update Empty Steps", WorkflowType.REQUEST, 10L);
                createDto.setSteps(List.of(buildStep(1, 10L)));
                WorkflowResponseDTO created = workflowService.create(createDto);

                UpdateWorkflowDTO dto = new UpdateWorkflowDTO();
                dto.setSteps(List.of());

                WorkflowResponseDTO result = workflowService.update(created.getId(), dto);

                assertThat(result.getSteps()).isEmpty();
                // Lấy danh sách steps nối với workflow này ra
                List<WorkflowStep> remainingSteps = workflowStepRepository.findAll().stream()
                        .filter(step -> step.getWorkflow().getId().equals(created.getId()))
                        .toList();

                // Vẫn còn 1 record trong DB (Do xóa mềm)
                assertThat(remainingSteps).hasSize(1);

                // DÒNG NÀY CỰC KỲ QUAN TRỌNG: Phải chứng minh nó ĐÃ BỊ XÓA MỀM
                assertThat(remainingSteps.get(0).getIsActive()).isFalse();
        }

        /**
         * Test Case ID: WF-TC20
         * Nhánh [N17]+[N18]+[N19]: update() với steps hợp lệ → lưu lại steps mới và cập
         * nhật isActive
         */
        @Test
        @DisplayName("[WF-TC20][N17-N19] update() - steps hợp lệ → thay steps cũ bằng steps mới")
        void tc20_update_withValidSteps_replacesAndPersistsSteps() {
                Workflow workflow = new Workflow();
                workflow.setName("Workflow Update Valid Steps");
                workflow.setType(WorkflowType.REQUEST);
                workflow.setDepartmentId(10L);
                workflow.setIsActive(true);
                workflow.setCreatedBy(CREATOR_USER_ID);
                workflow = workflowRepository.save(workflow);
                final Long workflowId = workflow.getId();

                UpdateWorkflowDTO dto = new UpdateWorkflowDTO();
                dto.setDescription("Mô tả workflow đã cập nhật");
                dto.setIsActive(false);
                dto.setSteps(List.of(buildStep(1, 10L), buildStep(2, 20L)));
                ...
                assertThat(result.getDescription()).isEqualTo("Mô tả workflow đã cập nhật");
                assertThat(result.getSteps()).hasSize(2);

                List<WorkflowStep> steps = workflowStepRepository.findAll().stream()
                                .filter(step -> step.getWorkflow().getId().equals(workflowId))
                                .sorted(java.util.Comparator.comparing(WorkflowStep::getStepOrder))
                                .toList();

                assertThat(steps).hasSize(2);
                assertThat(steps.get(0).getStepOrder()).isEqualTo(1);
                assertThat(steps.get(0).getApproverPositionId()).isEqualTo(10L);
                assertThat(steps.get(1).getStepOrder()).isEqualTo(2);
                assertThat(steps.get(1).getApproverPositionId()).isEqualTo(20L);
        }

        /**
         * Test Case ID: WF-TC21
         * Nhánh [N4] + nhánh false của toStepResponseDTO(): approverPositionId = null
         */
        @Test
        @DisplayName("[WF-TC21][N4] create() - step không có approverPositionId → vẫn tạo workflow và map step null")
        void tc21_create_stepWithNullApproverPositionId_skipsValidationAndMapsNull() {
                WorkflowStep step = new WorkflowStep();
                step.setStepOrder(1);
                step.setApproverPositionId(null);
                step.setIsActive(true);

                try {
                        java.lang.reflect.Method method = WorkflowService.class.getDeclaredMethod(
                                        "toStepResponseDTO", WorkflowStep.class, Map.class);
                        method.setAccessible(true);

                        WorkflowResponseDTO dto = new WorkflowResponseDTO();
                        com.example.workflow_service.dto.workflow.WorkflowStepResponseDTO result = (com.example.workflow_service.dto.workflow.WorkflowStepResponseDTO) method
                                        .invoke(
                                                        workflowService, step, Map.of());

                        assertThat(result.getApproverPositionId()).isNull();
                        assertThat(result.getApproverPositionName()).isNull();
        ...
        /**
         * Test Case ID: WF-TC22
         * Nhánh: dto.getSteps() không null nhưng tất cả approverPositionId = null
         * Mục tiêu: đường dẫn not calling userService.getPositionHierarchyOrdersByIds
         */
        @Test
        @DisplayName("[WF-TC22] update() - steps có approverPositionId null → bỏ qua validation")
        void tc22_update_stepsAllNullApprover_skipsHierarchyValidationAndPersists() {
                Workflow workflow = new Workflow();
                workflow.setName("Workflow Steps Null Approver");
                workflow.setType(WorkflowType.REQUEST);
                workflow.setDepartmentId(10L);
                workflow.setIsActive(true);
                workflow.setCreatedBy(CREATOR_USER_ID);
                ...
                UpdateWorkflowDTO dto = new UpdateWorkflowDTO();
                dto.setSteps(List.of(buildStep(1, null), buildStep(2, null)));

                // Should not throw because positionIds list will be empty
                WorkflowResponseDTO result = workflowService.update(workflowId, dto);
                ...
                List<WorkflowStep> steps = workflowStepRepository.findAll().stream()
                                ...
                                .toList();
                assertThat(steps).hasSize(2);
                assertThat(steps.get(0).getApproverPositionId()).isNull();
                assertThat(steps.get(1).getApproverPositionId()).isNull();
        }

        /**
         * Test Case ID: WF-TC23
         * Nhánh: positionHierarchyOrders.get(positionId) == null → ném CustomException
         */
        @Test
        @DisplayName("[WF-TC23] update() - missing hierarchyOrder → CustomException")
        void tc23_update_missingHierarchyOrder_throwsCustomException() {
                Workflow workflow = new Workflow();
                workflow.setName("Workflow Missing Hierarchy");
                workflow.setType(WorkflowType.REQUEST);
                workflow.setIsActive(true);
                workflow.setCreatedBy(CREATOR_USER_ID);
                workflow = workflowRepository.save(workflow);
                ...
                UpdateWorkflowDTO dto = new UpdateWorkflowDTO();
                dto.setSteps(List.of(buildStep(1, 999L)));

                // Mock userService to return empty map so hierarchyOrder == null
                when(userService.getPositionHierarchyOrdersByIds(anyList(), anyString()))
                                .thenReturn(Map.of());

                CustomException ex = assertThrows(CustomException.class,
                                () -> workflowService.update(workflowId, dto));
                assertTrue(ex.getMessage().contains("Không tìm thấy hierarchyOrder"));
        }

        /**
         * Test Case ID: WF-TC24
         * Nhánh: hierarchyOrder tăng dần (không hợp lệ) → ném CustomException
         */
        @Test
        @DisplayName("[WF-TC24] update() - invalid hierarchyOrder sequence → CustomException")
        void tc24_update_invalidHierarchyOrder_throwsCustomException() {
                Workflow workflow = new Workflow();
                workflow.setName("Workflow Invalid Hierarchy");
                workflow.setType(WorkflowType.REQUEST);
                workflow.setIsActive(true);
                workflow.setCreatedBy(CREATOR_USER_ID);
                workflow = workflowRepository.save(workflow);
                ...
                // Make hierarchy orders such that 20L has larger value than 10L -> invalid
                when(userService.getPositionHierarchyOrdersByIds(anyList(), anyString()))
                                .thenReturn(Map.of(10L, 1, 20L, 3));
